...
void main() {
  test('Color.empty() results in [ColorFailureEmpty]', () {
    final color = Color.empty();
    color.value.fold(
      (l) => expect(l, isA<ColorFailureEmpty>()),
      (r) => fail(r.toString()),
    );
  });
  ...
  test('Color.fromString("a") results in [TokenFailureEmpty]', () {
    ...
    color.value.fold(
      (l) => expect(l, isA<ColorFailureParsing>()),
      (r) => fail(r.toString()),
    );
  });
  ...
  test('Interval.value(0) results in [IntervalFailureMinimum]', () {
    ...
    expect(color.value, const Right<dynamic, int>(0xFFFFFFFF));
  });
}
